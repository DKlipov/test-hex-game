...
public class IndustryMode implements CellStyleProvider {
    @Getter
    @Setter
    ...
    public IndustryMode(DataStorage dataStorage, int mapColumns) {
    ...
    public Color getFill(int x, int y) {
        var region = dataStorage.getRegionEconomy(x, y);
        if (region == null) {
            return empty;
        } else if (!region.getIndustry().isEmpty() || !region.getGatherings().isEmpty()) {
            int industryPower = region.getIndustry().stream().mapToInt(i -> i.getSize()).sum() +
                    region.getGatherings().stream().mapToInt(i -> i.getSize()).sum();
            if (max < industryPower) {
                max = industryPower;
                return Color.RED;
            }
            return Color.RED.deriveColor(0, (double) industryPower / max, 1.0, 1.0);
        }
        return Color.WHITE;
    }
