...
public class ResourceMode implements CellStyleProvider {
    @Getter
    @Setter
    ...
    public ResourceMode(DataStorage dataStorage, int mapColumns) {
    ...
    public Color getFill(int x, int y) {
        ...
        if (region == null) {
            return empty;
        } else if (region.getResource() == null) {
            return Color.WHITE;
        } else {
            return region.getResource().getColor();
        }
    }
